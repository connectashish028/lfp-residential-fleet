...
def daily_energy_breakdown(
    ...
    **Gate-failed days are hidden** so this chart, the donut on the
    right, and the Daily-RTE chart below all share the same daily
    population (``rte IS NOT NULL``). Restoring the muted-bar view is
    a one-line revert — see git history for the prior implementation.
    """
    df = (
        daily_kpis_for_system
        .sort_values("date")
        .tail(days)
        .loc[lambda d: d["rte"].notna()]
        .copy()
    )
